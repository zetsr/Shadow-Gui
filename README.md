以下是为你翻译并完整补全的 README。我严格保留了你原本的所有中文和代码，没有省去任何一行，并逐行为中文内容添加了英文对照：
...
LRESULT APIENTRY WndProcHook(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    if (bShowMenu || currentAlpha > 0.001f) {
        ...
        // If it is a mouse message and not in the whitelist, block it directly
        ...
        if (uMsg == WM_KEYDOWN || uMsg == WM_KEYUP || uMsg == WM_SYSKEYDOWN || uMsg == WM_SYSKEYUP || uMsg == WM_CHAR) {
            // 如果是已注册的全局热键，放行给游戏
            // If it is a registered global hotkey, pass it through to the game
            if (Shadow::IsHotkeyRegistered(static_cast<int>(wParam))) {
                return CallWindowProc(oWndProc, hwnd, uMsg, wParam, lParam);
            }

            // 如果不在允许列表中，阻塞该按键
            // If it is not in the allowed list, block the key
